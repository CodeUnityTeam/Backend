...
class MinioService:
    ...
    def _ensure_bucket_templated(self) -> None:
        ...
        try:
            ...
            # 3. Применяем политику к бакету  # Политика применяется к бакету
            # при каждом запросе. Можно добавить флаг, что политика уже была
            # применения, и каждый раз не обращаться к апи MinIO
            s3_client.put_bucket_policy(
                Bucket=self.bucket_name,
                Policy=json.dumps(public_read_policy),
            )
        except Exception:  # noqa: BLE001  # Будет логгирование?
            pass
    ...
    def upload_file(self, cloud_path: str, file_obj: UploadedFile) -> str:
        ...
        return f'http://{self.storage.custom_domain}/{saved_name}'  # Мб стоит
                                              # протокол брать из endpoint_url?
