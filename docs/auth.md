...
Перейти по ссылке из письма (GET-запрос на фронтенд, который затем дёргает бэкенд для подтверждения). Ручка подтверждения — стандартная `dj_rest_auth`: **`POST /auth/registration/verify-email/`** с ключом.
...
> **Важно:** `access` и `refresh` — это JWT-токены. `access` живёт 7 дней, `refresh` — тоже 7 дней (настройка `SIMPLE_JWT`). Токены также устанавливаются в cookies: `access-token` и `refresh-token`.
...
> **Важно:** Поле `code` автоматически декодируется из URL-encoding на бэкенде.
...
> **Важно:** Кастомный сериализатор принимает одно поле `password` (без `new_password1`/`new_password2`). Поле `old_password` опционально — если передать неправильный старый пароль, смена всё равно сработает (особенность реализации).

---

## 6. Профиль пользователя

### 6.1. Получить/редактировать свой профиль
...
> **Поля профиля:** `first_name`, `last_name`, `phone_number`, `additional_contact`, `country`, `city`, `soft_skills`, `about_me`, `projects_relation`, `skills`, `specializations`, `workformats`. Email и role — read-only.

### 6.2. Удалить аккаунт (мягкое удаление)
...
> Флаги `is_active` и `is_agreed_to_terms` переводятся в `False`. При повторной регистрации с тем же email аккаунт восстанавливается.
...
### 6.6. Посмотреть чужой профиль

**`GET /profile/{user_id}/`**

**Заголовок:** `Authorization: Bearer <access-токен>`

### 6.7. Список пользователей
...
## 7. Последовательность действий (сценарии)
